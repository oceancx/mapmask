...
// 		DWORD      biSize;				// 本结构所占用字节数，即sizeof(BITMAPINFOHEADER);
// 		LONG       biWidth;				// 位图宽度（单位：像素）
// 		LONG       biHeight;			// 位图高度（单位：像素）
// 		WORD       biPlanes;			// 目标设备的级别，必须为1
// 		WORD       biBitCount;			// 像素的位数（每个像素所需的位数，范围：1、4、8、24、32）
// 		DWORD      biCompression;		// 压缩类型（0：不压缩 1：BI_RLE8压缩类型 2：BI_RLE4压缩类型）
// 		DWORD      biSizeImage;			// 位图大小（单位：字节）
// 		LONG       biXPelsPerMeter;		// 水平分辨率(像素/米)
// 		LONG       biYPelsPerMeter;		// 垂直分辨率(像素/米)
// 		DWORD      biClrUsed;			// 位图实际使用的彩色表中的颜色索引数
// 		DWORD      biClrImportant;		// 对图象显示有重要影响的颜色索引的数目
// 	}BITMAPINFOHEADER;
